...
(function() {
	updateProfile = function(v,frm) {
		...
		fcom.ajax(fcom.makeUrl('traveler', 'updateProfile'),fcom.frmData(frm), function(json) {
			json = $.parseJSON(json);
			if(json.status == '1'){
	...
	removeImage = function(v,frm) {
		confirmbox('Do You Want To Remove ?',function(outcome){
			if(outcome){
				jsonStrictNotifyMessage();
				fcom.ajax(fcom.makeUrl('traveler', 'removeImage'),{}, function(json) {
				json = $.parseJSON(json);
				if(json.status == '1'){
	...
	updatePassword = function(v,frm) {
		...
		if (!v.isValid()) return false;
		jsonStrictNotifyMessage();
		fcom.ajax(fcom.makeUrl('traveler', 'passwordSetup'),fcom.frmData(frm), function(json) {
			json = $.parseJSON(json);
			if(json.status == '1'){
				jsonSuccessMessage(json.msg);
				$(frm)[0].reset();
	...
	profileForm = function(tab){
		...
		fcom.ajax(fcom.makeUrl('traveler', 'step'),{'tab':tab}, function(json) {
			json = $.parseJSON(json);
			if(json.status == '1'){
				jsonRemoveMessage();
	...
	sumbmitProfileImage = function(){
		...
		delegation: true,
		url:fcom.makeUrl('traveler','setupProfileImage'),
	...
	updateEmail = function(v,frm) {
		...
		if (!v.isValid()) return false;
		jsonStrictNotifyMessage();
		fcom.ajax(fcom.makeUrl('traveler', 'updateEmail'),fcom.frmData(frm), function(json) {
			json = $.parseJSON(json);
			if(json.status == '1'){
				jsonSuccessMessage(json.msg);
				$(frm)[0].reset();
