...
<form id = "time-slot-form" method = 'post' onsubmit = 'actionStep6();return false;' action = '<?php echo FatUtility::generateUrl('hostactivity', 'setup6') ?>' class="form form--default form--horizontal">
    ...
    <div class="form-element">
        <label class="form-element__label">
            <?php echo Info::t_lang('SERVICE') ?>
            <a href="#available-instruction" class="available-instruction-popup">
    ...
    <div class="form-element">
        <label class="form-element__label">
            <?php echo Info::t_lang('BULK_ENRIES') ?>
            <a href="#bulk-entry-instruction" class="bulk-entry-instruction-popup">
                <svg class="icon icon--info   ">
                <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#icon-info"></use>
                </svg>
            </a>
        ...
        <table width="100%" class="d-calendar__view">
            ...
            <tbody class="d-calendar__view__body">
                ...
                foreach ($calendar as $k => $cal) {
                    if ($k % 7 == 0) {
                        echo '<tr class="d-calendar__week">';
                    }
                    ?>
                                ...
                                foreach ($cal['events'] as $event) {
                                    ...
                                    <?php if ($event['activityevent_anytime'] == 0) { ?>
                                        <li><span class="time <?php
                                            if ($event['activityevent_confirmation_requrired'] == 1) {
                                                echo "required";
                                            }
                                            ?>"><?php echo date('H:i', strtotime($event['activityevent_time'])) ?>
                                                <a href="<?php echo FatUtility::generateUrl('hostactivity', 'editEvent', array($event['activityevent_id'])) ?>" class="time__edit modaal-ajax"  >EDIT</a> <a href="javascript:;" class="time__del" onclick="deleteEvent(this,<?php echo $event['activityevent_id'] ?>)">x</a></span>
                                        </li>
                                        <?php
                                    } else {
                                        $addNew = false;
                                        ...
                                        <li><span class="time <?php
                                            ...
                                            ?>"><?php echo Info::t_lang('FULL_DAY') ?><a href="<?php echo FatUtility::generateUrl('hostactivity', 'editEvent', array($event['activityevent_id'])) ?>" class="time__edit modaal-ajax" >EDIT</a><a href="javascript:;" class="time__del" onclick="deleteEvent(this,<?php echo $event['activityevent_id'] ?>)">x</a></span></li>
                                                  <?php
                                              }
                                          }
                                      }
...
<input type="button" value="<?php echo Info::t_lang("PREV_STEP"); ?>" name="button" data-fatreq="{&quot;required&quot;:false}" title="" ng-click="prevPage(3);" class="button button--small button--fill button--dark fl--left" style="margin-top:1.25em;">	
<input type="button" value="<?php echo Info::t_lang("NEXT_STEP"); ?>" name="button" data-fatreq="{&quot;required&quot;:false}" title="" ng-click="nextPage(3);" class="button button--small button--fill button--dark fl--right" style="margin-top:1.25em;">
