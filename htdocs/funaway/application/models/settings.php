...
class Settings extends MyAppModel {
    ...
    function update($data){
		...
		foreach($data as $key => $val){
			if(in_array($key, $accept_keys, true)){
				...
				if (!is_array($val)) {
					$arr = array('conf_var' => $key ,'conf_val'=>$val,'conf_serialized'=>0 );
				}else{
					$arr = array('conf_var' => $key ,'conf_val'=>serialize($val),'conf_serialized'=>1 );
				}
				$this->db->insert_from_array('tbl_configurations',$arr, false ,false, $arr);
	...
	function send_smtp_test_email($smtp_arr){
	 try {
		 $email = sendMailTpl(Settings::getSetting("CONF_ADMIN_EMAIL"), 'smtp_settings', array(
...
//Call this function with argument = absolute path of file or directory name.
	...
	function send_test_email(){
		 ...
		 try {
			  ...
			  $email = sendMailTpl(Settings::getSetting("CONF_ADMIN_EMAIL"), 'mail_settings', array(
						'{website_name}' => Settings::getSetting("CONF_WEBSITE_NAME")));
				if($email){
					return true;
				}else{
