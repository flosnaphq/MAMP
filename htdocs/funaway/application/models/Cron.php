...
class Cron extends MyAppModel {
	...
	public function __construct($cronId = 0) {
		...
		$this->objMainTableRecord->setSensitiveFields(array('ac_balance'));
	}

	public static function clearOldLog() {
		FatApp::getDb ()->deleteRecords ( static::DB_TBL_LOG, array (
	...
	public static function getAllRecords($activeOnly = true, $id = 0) {
		...
		if ( $activeOnly ) {
			$srch->addCondition('cron_active', '=', 1);
		}
	...
	public function markStarted() {
		...
		FatApp::getDb()->insertFromArray(static::DB_TBL_LOG, array(
	...
	public function markFinished($logId, $message) {
		...
		$db->updateFromArray ( static::DB_TBL_LOG, array (
	...
	private function canStart() {
		$db = FatApp::getDb();
		$rs = $db->query('SELECT * FROM ' . static::DB_TBL_LOG . ' WHERE cronlog_cron_id = ' . $this->mainTableRecordId . ' 
