					...
					switch ($key){
						...
						case 'action':
							...
							if($canView){
								...
								$li->appendElement('a', array('href'=>'Javascript:popupView("'.FatUtility::generateUrl('merchants','view',array('user_id'=>$row['user_id'])).'");', 'class'=>'button small green', 'title'=>'View detail'),'<i class="ion-eye icon"></i>', true);	
							}
							if($canEdit){
								$li = $ul->appendElement("li");
								$li->appendElement('a', array('href'=>FatUtility::generateUrl('restaurants','index',array('user_id'=>$row['user_id'])), 'class'=>'button small green', 'title'=>'View Restaurant'),'<i class="ion-android-folder-open icon"></i>', true);	
							}
							if($canEdit){
								$li = $ul->appendElement("li");
							...
							if($canViewOrder){
								$li = $ul->appendElement("li");
								$li->appendElement('a', array('href'=>FatUtility::generateUrl('orders','listing',array('order_type'=>'merchant','user_id'=>$row['user_id'])), 'class'=>'button small green', 'title'=>'View Orders'),'<i class="ion-clipboard icon"></i>', true);	
							}
							if($canViewMerchantSubUser){
								$li = $ul->appendElement("li");
								$li->appendElement('a', array('href'=>FatUtility::generateUrl('merchants','sub-users',array('merchant_id'=>$row['user_id'])), 'class'=>'button small green', 'title'=>'View Sub user'),'<i class="ion-person-stalker icon"></i>', true);	
							}
							break;
						default:
