...
(function() {
	...
	listing = function(page){
		...
		fcom.ajax(fcom.makeUrl('withdrawal-requests', 'lists', [page]), data, function(json) {
			json = $.parseJSON(json);
			if("1" == json.status){
...
search = function(form){
	...
	fcom.ajax(fcom.makeUrl('withdrawal-requests', 'lists'), fcom.frmData(form), function(json) {
			json = $.parseJSON(json);
			if("1" == json.status){
				$("#listing").html(json.msg);
				$('#clearSearch').show();
...
getForm = function(request_id){
	...
	fcom.ajax(fcom.makeUrl('withdrawal-requests', 'form'), {"request_id":request_id}, function(json) {
			json = $.parseJSON(json);
			if("1" == json.status){
