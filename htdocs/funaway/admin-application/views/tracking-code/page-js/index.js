...
listing = function(page){
	...
	moveToTop();
	fcom.ajax(fcom.makeUrl('trackingCode', 'listing', [page]), data, function(json) {
			json = $.parseJSON(json);
			if("1" == json.status){
...
getForm = function(tcode_id){
	if(typeof tcode_id === undefined){
		tcode_id = 0;
	}
	
	fcom.ajax(fcom.makeUrl('trackingCode', 'form'), {"tcode_id":tcode_id}, function(json) {
			json = $.parseJSON(json);
			if("1" == json.status){
...
search = function(form){
	...
	fcom.ajax(fcom.makeUrl('trackingCode', 'listing'), fcom.frmData(form), function(json) {
			json = $.parseJSON(json);
			if("1" == json.status){
				$("#listing").html(json.msg);
				$('#clearSearch').show();
