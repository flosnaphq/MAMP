...
<?php	
			$arr_flds = array(
				'listserial'=>'Sr no.',
				TrackingCode::DB_TBL_PREFIX.'name'=>'Name',
				TrackingCode::DB_TBL_PREFIX.'status' => 'Status',
				'action' => 'Action',
			);
			...
			foreach ($arr_listing as $sn=>$row){
				...
				if($row[TrackingCode::DB_TBL_PREFIX.'status']==0) {
				...
				foreach ($arr_flds as $key=>$val){
					...
					switch ($key){
						...
						case TrackingCode::DB_TBL_PREFIX.'status':
						...
						case 'action':
							...
							if($canEdit){
								...
								$li->appendElement('a', array('href'=>"javascript:;", 'class'=>'button small green', 'title'=>'Edit',"onclick"=>"getForm(".$row['tcode_id'].")"),'<i class="ion-edit icon"></i>', true);	
							}
							if($canView){
								$li = $ul->appendElement("li");
								$li->appendElement('a', array('href'=>'Javascript:popupView("'.FatUtility::generateUrl('trackingCode','view',array('tcode_id'=>$row['tcode_id'])).'");', 'class'=>'button small green', 'title'=>'View detail'),'<i class="ion-eye icon"></i>', true);	
							}
							break;
						default:
