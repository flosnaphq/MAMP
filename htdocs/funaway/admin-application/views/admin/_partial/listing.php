				...
				foreach ($arr_flds as $key=>$val){
					...
					switch ($key){
						...
						case 'action':
							...
							if($canEdit){
								$li = $ul->appendElement("li");
								$li->appendElement('a', array('href'=>"javascript:;", 'class'=>'button small green', 'title'=>'Manage Password',"onclick"=>"getForgotPasswordForm(".$row['admin_id'].")"),'<i class="ion-unlocked icon"></i>', true);
							}
							if($canView){
								$li = $ul->appendElement("li");
