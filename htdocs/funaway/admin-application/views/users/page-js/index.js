...
(function() {
	...
	listing = function(tab, page){
		...
		moveToTop();
		fcom.ajax(fcom.makeUrl('users', 'lists', [page,tab]), data, function(json) {
			json = $.parseJSON(json);
			if("1" == json.status){
	...
	search = function(form,tab){
		...
		fcom.ajax(fcom.makeUrl('users', 'lists',[1,tab]), fcom.frmData(form), function(json) {
				json = $.parseJSON(json);
				if("1" == json.status){
					$("#listing").html(json.msg);
					$('#clearSearch').show();
	...
	getForm = function(user_id){
		...
		fcom.ajax(fcom.makeUrl('users', 'form'), {"user_id":user_id}, function(json) {
				json = $.parseJSON(json);
				if("1" == json.status){
	...
	getPasswordForm = function(user_id){
		...
		fcom.ajax(fcom.makeUrl('users', 'passwordForm'), {"user_id":user_id}, function(json) {
				json = $.parseJSON(json);
				if("1" == json.status){
					$("#form-tab").html(json.msg);
					jsonSuccessMessage('Form Load');
