			...
			foreach ($arr_listing as $sn=>$row){
				...
				foreach ($arr_flds as $key=>$val){
					...
					switch ($key){
						...
						case 'block_id':
							$td->appendElement('plaintext', array(), '[block blkid='.$row[$key].']');
							break;
						...
						case 'action':
							...
							if($canEdit){
								...
								$li->appendElement('a', array('href'=>"javascript:;", 'class'=>'button small green', 'title'=>'Edit',"onclick"=>"getForm(".$row['block_id'].")"),'<i class="ion-edit icon"></i>', true);	
							}
							if($canView){
								$li = $ul->appendElement("li");
								$li->appendElement('a', array('href'=>'Javascript:popupView("'.FatUtility::generateUrl('blocks','view',array('block_id'=>$row['block_id'])).'");', 'class'=>'button small green', 'title'=>'View detail'),'<i class="ion-eye icon"></i>', true);	
							}
							break;
						default:
