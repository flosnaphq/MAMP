...
<?php	
			$arr_flds = array(
				...
				'faq_display_order'=>'DIsplay Order',
				'faq_active' => 'Status',
				'action' => 'Action',
			);
				...
				foreach ($arr_flds as $key=>$val){
					...
					switch ($key){
						...
						case 'faq_display_order':
							if($canEdit){
								$td->appendElement('input', array('value'=>$row[$key],'onblur'=>'chanageOrder("'.$row['faq_id'].'",this)'));
							}
							else{
						...
						case 'action':
							...
							if($canEdit){
								...
								$li->appendElement('a', array('href'=>"javascript:;", 'class'=>'button small green', 'title'=>'Edit',"onclick"=>"getForm(".$row['faq_faqcat_id'].", ".$row['faq_id'].")"),'<i class="ion-edit icon"></i>', true);	
							}
							if($canView){
								$li = $ul->appendElement("li");
								$li->appendElement('a', array('href'=>'Javascript:popupView("'.FatUtility::generateUrl('faqs','faq-view',array('faq_id'=>$row['faq_id'])).'");', 'class'=>'button small green', 'title'=>'View detail'),'<i class="ion-eye icon"></i>', true);	
							}
