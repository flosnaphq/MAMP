					...
					switch ($key){
						...
						case 'action':
							...
							if($canEdit){
								...
								$li->appendElement('a', array('href'=>"javascript:;", 'class'=>'button small green', 'title'=>'Edit',"onclick"=>"getForm(".$row['city_id'].")"),'<i class="ion-edit icon"></i>', true);	
							}
							if($canView){
								$li = $ul->appendElement("li");
								$li->appendElement('a', array('href'=>'Javascript:;popupView("'.FatUtility::generateUrl('locations','view-city',array('city_id'=>$row['city_id'])).'");', 'class'=>'button small green', 'title'=>'View detail'),'<i class="ion-eye icon"></i>', true);	
							}
							if($canEdit){
								$li = $ul->appendElement("li");
								$li->appendElement('a', array('href'=>FatUtility::generateUrl('locations','regions',array('city_id'=>$row['city_id'])), 'class'=>'button small green', 'title'=>'Regions'),'<i class="ion-location icon"></i>', true);	
							}
							if($canEdit){
								$li = $ul->appendElement("li");
								$li->appendElement('a', array('href'=>"javascript:;", 'class'=>'button small green', 'title'=>'Edit',"onclick"=>"deleteCity(".$row['city_id'].")"),'<i class="ion-ios-close-outline icon"></i>', true);	
							}
