...
<?php	
			$arr_flds = array(
				'listserial'=>'Sr no.',
				'cms_name'=>'Title',
				'cms_sub_heading'=>'Sub Heading',
				'cms_type'=>'Page Type',
				'cms_display_order'=>'Display Order',
				//'cmsposition_position_id' => 'Position',
				'cms_active' => 'Status',
				'action' => 'Action',
			);
				...
				foreach ($arr_flds as $key=>$val){
					...
					switch ($key){
						...
						case 'cms_active':
							...
							break;
						case 'cmsposition_position_id':
							$td->appendElement('plaintext', array(), is_array(Info::getCmsPositions($row[$key]))?'none':Info::getCmsPositions($row[$key]));
							break;
						case 'cms_type':
							$td->appendElement('plaintext', array(), Info::getCmsTypeByKey($row[$key]));
							break;
						case 'cms_display_order':
							if($canEdit){
								$td->appendElement('input', array('value'=>$row[$key],'onblur'=>'changeOrder("'.$row['cms_id'].'",this)','class'=>'text-display-order'));
							}
							else{
						...
						case 'action':
							...
							if($canEdit){
								...
								$li->appendElement('a', array('href'=>"javascript:;", 'class'=>'button small green', 'title'=>'Edit',"onclick"=>"getForm(".$row['cms_id'].")"),'<i class="ion-edit icon"></i>', true);	
							}
							if($canView){
								$li = $ul->appendElement("li");
								$li->appendElement('a', array('href'=>'Javascript:popupView("'.FatUtility::generateUrl('cms','view',array('cms_id'=>$row['cms_id'])).'");', 'class'=>'button small green', 'title'=>'View detail'),'<i class="ion-eye icon"></i>', true);	
							}
							break;
						default:
