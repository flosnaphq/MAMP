...
listing = function(page){
	...
	moveToTop();
	fcom.ajax(fcom.makeUrl('sms-template', 'listing', [page]), data, function(json) {
			json = $.parseJSON(json);
			if("1" == json.status){
...
getForm = function(smstpl_id){
	if(typeof smstpl_id === undefined){
		smstpl_id = 0;
	}
	
	fcom.ajax(fcom.makeUrl('sms-template', 'form'), {"smstpl_id":smstpl_id}, function(json) {
			json = $.parseJSON(json);
			if("1" == json.status){
...
search = function(form){
	...
	fcom.ajax(fcom.makeUrl('sms-template', 'listing'), fcom.frmData(form), function(json) {
			json = $.parseJSON(json);
			if("1" == json.status){
				$("#listing").html(json.msg);
				$('#clearSearch').show();
