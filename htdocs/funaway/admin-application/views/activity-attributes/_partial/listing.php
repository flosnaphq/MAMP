...
<?php	
			$arr_flds = array(
				'listserial'=>'Sr no.',
				ActivityAttributes::DB_TBL_PREFIX.'caption'=>'Caption',
				ActivityAttributes::DB_TBL_PREFIX.'file_required'=>'File Required',
				ActivityAttributes::DB_TBL_PREFIX.'status' => 'Status',
				'action' => 'Action',
			);
			...
			foreach ($arr_listing as $sn=>$row){
				...
				if($row[ActivityAttributes::DB_TBL_PREFIX.'status']==0) {
				...
				foreach ($arr_flds as $key=>$val){
					...
					switch ($key){
						...
						case ActivityAttributes::DB_TBL_PREFIX.'status':
							$td->appendElement('plaintext', array(), Info::getStatusByKey($row[$key]));
							break;
						case ActivityAttributes::DB_TBL_PREFIX.'file_required':
							$td->appendElement('plaintext', array(), Info::getIsValue($row[$key]));
							break;
						...
						case 'action':
							...
							if($canEdit){
								...
								$li->appendElement('a', array('href'=>"javascript:;", 'class'=>'button small green', 'title'=>'Edit',"onclick"=>"getForm(".$row[ActivityAttributes::DB_TBL_PREFIX.'id'].")"),'<i class="ion-edit icon"></i>', true);	
							}
