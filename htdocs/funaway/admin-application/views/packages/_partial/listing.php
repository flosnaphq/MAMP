					...
					switch ($key){
						...
						case 'action':
							...
							if($canEdit){
								...
								$li->appendElement('a', array('href'=>"javascript:;", 'class'=>'button small green', 'title'=>'Edit',"onclick"=>"getForm(".$row['package_id'].")"),'<i class="ion-edit icon"></i>', true);	
							}
							if($canView){
								$li = $ul->appendElement("li");
								$li->appendElement('a', array('href'=>'Javascript:popupView("'.FatUtility::generateUrl('packages','view-package',array('package_id'=>$row['package_id'])).'");', 'class'=>'button small green', 'title'=>'View detail'),'<i class="ion-eye icon"></i>', true);	
							}
							
							$li = $ul->appendElement("li");
							$li->appendElement('a', array('href'=>FatUtility::generateUrl('packages','options',array('package_id'=>$row['package_id'])), 'class'=>'button small green', 'title'=>'Options'),'<i class="ion-android-menu icon"></i>', true);	
