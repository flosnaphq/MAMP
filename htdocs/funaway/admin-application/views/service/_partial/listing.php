...
<?php	
			$arr_flds = array(
				...
				'service_name'=>'Name',
				'service_id'=>'Image',
				'service_display_order'=>'Display Order',
			//	'service_featured'=>'Featured',
				'service_active' => 'Status',
				'action' => 'Action',
			);
			if(!$canEdit && $service_parent_id > 0){
				...
				foreach ($arr_flds as $key=>$val){
					...
					switch ($key){
						...
						case 'service_active':
							...
							break;
						case 'service_id':
							$td->appendElement('img', array("src"=>FatUtility::generateUrl("image","service",array($row["service_id"],75,75),CONF_BASE_DIR)."?".Info::timestamp()));
							break;
						case 'service_featured':
								if($canEdit){
									$active = $row[$key] !=1?'active':'';
									$toggle = '<label class="statustab addmarg '.$active.'" onclick="setFeatured(this, \''.$row['service_id'].'\')">
							...
							break;
						case 'service_display_order':
							if($canEdit){
								$td->appendElement('input', array('value'=>$row[$key],'onblur'=>'chanageServiceOrder("'.$row['service_id'].'",this)'));
							}
							else{
						...
						case 'action':
							...
							if($canEdit){
								...
								$li->appendElement('a', array('href'=>"javascript:;", 'class'=>'button small green', 'title'=>'Edit',"onclick"=>"getForm(".$row['service_id'].")"),'<i class="ion-edit icon"></i>', true);	
							}
						/* 	if($canView){
								$li = $ul->appendElement("li");
								$li->appendElement('a', array('href'=>'Javascript:popupView("'.FatUtility::generateUrl('service','view',array('service_id'=>$row['service_id'])).'");', 'class'=>'button small green', 'title'=>'View detail'),'<i class="ion-eye icon"></i>', true);	
							} */
							if($canView && $service_parent_id ==0){
								$li = $ul->appendElement("li");
								$li->appendElement('a', array('href'=>FatUtility::generateUrl('service','services',array('service_id'=>$row['service_id'])), 'class'=>'button small green', 'title'=>'View detail'),'<i class="ion-android-menu icon"></i>', true);	
							}
							break;
						default:
