...
getForm = function(service_id){
	...
	fcom.ajax(fcom.makeUrl('service', 'form'), {"service_id":service_id}, function(json) {
			json = $.parseJSON(json);
...
chanageServiceOrder = function(service_id, input){
	fcom.ajax(fcom.makeUrl('service', 'service-display-setup',[service_id]), {service_display_order:input.value,service_id:service_id}, function(json) {
			json = $.parseJSON(json);
			if("1" == json.status){
